...
"""Capability: recommend_next_sensor — advisory next-sensor recommendation."""
...
_SENSOR_META = {
    "radar": {"fills": "subsurface / micro-Doppler confirmation",
              "when_weak": "Limited through-rubble or motion-band evidence."},
    "thermal": {"fills": "thermal contrast / hotspot corroboration",
                "when_weak": "Limited thermal delta or hotspot coverage."},
    "acoustic": {"fills": "tap / voice-pattern corroboration",
                 "when_weak": "Limited acoustic confirmation."},
    "starlink": {"fills": "overhead / RF-associated context",
                 "when_weak": "Missing or weak overhead observation context."},
    "celltower": {"fills": "coarse infrastructure-associated location context",
                  "when_weak": "No cell-tower enrichment on this cell."},
}

def recommend_next_sensor(backend, params, *, detections: DetectionReadPort):
    ...
    return CapabilityResult(
        ...
        metadata={"access": "read-only", "port": "DetectionReadPort", "operational_tasking": False},
    )

def _scores(cell):
    return {name: float(cell.get(f"{name}_score") or 0) for name in _SENSOR_META}

def _prefer_order(names):
...
def _recommend(cell):
    ...
    if len(active) <= 1 and weak_or_missing:
        ...
        candidates.append((pick, "high", "Corroboration from a second modality",
            f"Only {len(active)} active modality(ies); multi-sensor agreement is weak."))
    if fused >= 0.4 and scores.get("radar", 0) < 0.25:
        candidates.append(("radar", "high", "Subsurface / micro-Doppler confirmation",
            "Fused confidence is elevated but radar contribution is low."))
    if scores.get("thermal", 0) < 0.25 and fused >= 0.3:
        candidates.append(("thermal", "medium", "Thermal contrast corroboration",
            _SENSOR_META["thermal"]["when_weak"]))
    if scores.get("acoustic", 0) < 0.25 and fused >= 0.3:
        candidates.append(("acoustic", "medium", "Acoustic pattern corroboration",
            _SENSOR_META["acoustic"]["when_weak"]))
    if scores.get("starlink", 0) < 0.25:
        candidates.append(("starlink", "medium" if fused >= 0.4 else "low",
            "Overhead / RF-associated context", _SENSOR_META["starlink"]["when_weak"]))
    if scores.get("celltower", 0) < 0.25 and fused < 0.5:
        candidates.append(("celltower", "low", "Coarse location enrichment",
            _SENSOR_META["celltower"]["when_weak"]))
    if not candidates:
        ordered = sorted(scores.items(), key=lambda kv: kv[1])
        pick = ordered[0][0]
        candidates.append((pick, "low", f"Incremental improvement on weakest modality ({pick})",
            "Evidence coverage is relatively complete; residual value is incremental."))

    priority_rank = {"high": 0, "medium": 1, "low": 2}
    candidates.sort(key=lambda c: (priority_rank.get(c[1], 9), _prefer_order_index(c[0])))
    ...
    return {
        ...
        "rationale": reason,
        "why_selected": f"{sensor} was selected because: {reason} It addresses: {_SENSOR_META[sensor]['fills']}.",
